...
struct ContentView: View {
	...
	@State private var onboardingStateEvaluated: Bool = false
    ...
    var body: some View {
		ZStack(alignment: .top) {
			...
			let activeBackgroundStyle = nonLandingBackgroundStyle
		...
		.onAppear {
			...
			evaluateOnboardingStateIfNeeded()
		}
		.onOpenURL { url in
			handleIncomingDeepLink(url)
				...
				onDeclined: {
					#if os(macOS)
						AppVisibilityCoordinator.shared.performOnboardingDeclineQuit()
					#endif
						}
					)
	...
	private func handleIncomingDeepLink(_ url: URL) {
		guard let route = mapDeepLinkToNavigation(url) else { return }
		selection = route
		isNavigationPresented = false
		inspector.hide()
	}

	private func mapDeepLinkToNavigation(_ url: URL) -> NavigationItem? {
		guard url.scheme?.lowercased() == "juice" else { return nil }
		let host = url.host?.lowercased() ?? ""
		let path = url.path.lowercased()
		let combined = "\(host)\(path)"
		if combined == "settings/environments" || combined == "settings" {
			return .settings
		}
		if combined == "updates" {
			return .updates
	...
	private func evaluateOnboardingStateIfNeeded() {
		guard !onboardingStateEvaluated else { return }
		onboardingStateEvaluated = true
			...
			if #available(macOS 26.0, iOS 26.0, *) {
				GlassEffectContainer(spacing: 30) {
					HStack(spacing: 35) {
						...
						icon: isPresented ? "sidebar.right" : "chevron.left.square",
...
private struct InspectorOverlayToolbarControl: View {
	...
	var body: some View {
		...
		let expandIcon = resolvedSymbolName(
			preferred: "sidebar.right",
			fallback: "chevron.left.square"
		)
		return ZStack(alignment: .topTrailing) {
			Button {
				...
				Image(systemName: inspector.isPresented ? collapseIcon : expandIcon)
					.font(.system(size: 14, weight: .regular))
					.frame(width: 28, height: 22)
					.contentShape(shape)
			}
