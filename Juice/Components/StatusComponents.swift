...
public struct Pill: View {
	...
	public init(_ value: String, color: Color? = nil, help: String? = nil) {
		self.value = value
		self.style = .solid(color)
		self.helpText = help
	}

	public init(_ value: String, style: PillStyle, help: String? = nil) {
		self.value = value
		self.style = style
		self.helpText = help
	}
	...
	public var body: some View {
		...
		case .solid(let color):
			...
			Text(value)
				...
				.juiceHelp(helpText ?? value)
		case .juiceGradient:
			Text(value)
				.foregroundStyle(.white)
				.font(.system(.footnote, weight: .semibold))
				.shadow(color: .black.opacity(0.22), radius: 0.8, x: 0, y: 0.5)
				.padding(.vertical, 2)
				.padding(.horizontal, 6)
				.background {
					shape.fill(LinearGradient.juice.opacity(0.74))
				}
				.overlay {
					shape.strokeBorder(.white.opacity(0.16), lineWidth: 0.8)
				}
				.juiceHelp(helpText ?? value)
		}
	}
...
struct InfoBadge: View {
	...
	var body: some View {
		...
		Text("\(count)")
			...
			.foregroundStyle(
				GlassThemeTokens.statusTextColor(.error, for: context)
			)
			.padding(.vertical, 2)
			.padding(.horizontal, 6)
...
struct StatCard: View {
	...
	@State private var animationRunToken: Int = 0
	let title: String
	let value: String
	let isLoading: Bool
	...
	private var numericCharacters: [Character]? {
	...
	var body: some View {
		...
		let content = VStack(alignment: .leading, spacing: 10) {
			...
			if isLoading {
				...
				if let digits = numericCharacters {
					HStack(spacing: 0) {
						ForEach(Array(digits.enumerated()), id: \.offset) {
							index,
							character in
	...
	private func runFlipYAnimation() {
		guard !isLoading, numericCharacters != nil else { return }
		animationRunToken += 1
		let token = animationRunToken
		flipX = 0
		DispatchQueue.main.async {
			guard token == animationRunToken else { return }
...
#Preview("StatCard Preview") {
	HStack(spacing: 20) {
		...
		StatCard(title: "Recipes", value: "0", isLoading: true)
	}
	.padding()
...
struct ThinkingIndicator: View {
	...
	var body: some View {
		TimelineView(.animation(minimumInterval: 1.0 / 30.0, paused: false)) {
			context in
			let elapsed =
				context.date.timeIntervalSinceReferenceDate - startTime
			let phraseDuration: TimeInterval = 2.0
			let phraseProgress =
				(elapsed.truncatingRemainder(dividingBy: phraseDuration))
				/ phraseDuration
			...
			HStack(spacing: 10) {
				...
				HStack(spacing: 0) {
					ForEach(Array(phrase.enumerated()), id: \.offset) {
						index,
						letter in
						animatedLetter(
							index: index,
	...
	private func phrase(at elapsed: TimeInterval, phraseDuration: TimeInterval)
		-> String
	{
	...
	private func pulseValue(elapsed: TimeInterval, duration: TimeInterval)
		-> Double
	{
