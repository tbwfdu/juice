...
struct LiquidGlassSegmentedPickerExtraLarge<Tag: Hashable>: View {
	// MARK: - Item
	...
	return VStack(spacing: 16) {
		HStack {
			...
			LiquidGlassSegmentedPicker(
				items: [
					.init(title: "Form", icon: "list.bullet.indent", tag: .queue),
					.init(
						title: "JSON",
...
#Preview("LiquidGlassSegmentedPickerXL") {
	enum SampleTab: Hashable {
		case queue
		case results
		case history
	...
	return VStack(spacing: 16) {
		HStack {
			Spacer()
			LiquidGlassSegmentedPickerExtraLarge(
				items: [
