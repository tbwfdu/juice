...
struct QueuePanelContent<QueueContent: View, ResultsContent: View>: View {
	...
	@State private var panelHeaderHeight: CGFloat = 120
		...
		ZStack(alignment: .center) {
			VStack(alignment: .leading, spacing: 12) {
				...
				panelContainer {
					...
					.animation(.timingCurve(0.25, 0.1, 0.25, 1.0, duration: 0.25), value: tab)
				}
			}
	...
	private func panelContent<Content: View>(
		...
		let titleBlock = VStack(alignment: .leading, spacing: 0) {
			Text(title)
				...
				.truncationMode(.tail)
				.juiceFullValueHelp(fullValue: title)
			Text(countText)
				.font(.subheadline)
				.foregroundStyle(.secondary)
				.lineLimit(1)
				.truncationMode(.tail)
				.juiceFullValueHelp(fullValue: countText)
		}
		...
		if #available(macOS 26.0, iOS 26.0, *) {
			HStack(spacing: 8) {
				if !actionTitle.isEmpty {
					...
					.juiceHelp(HelpText.Queue.clearResults)
...
struct QueueBottomActions: View {
	...
	init(
		...
		showsSecondaryAction: Bool = true,
		isEnabled: Bool,
		queueCount: Int,
	...
	var body: some View {
		HStack {
			Spacer()
			ExpandableJuiceMenuButton_AvailabilityAdapter(
...
private struct BouncingNoticeText: View {
	...
	private var noticeTextColor: Color {
		GlassThemeTokens.textPrimary(
			for: GlassStateContext(colorScheme: colorScheme, isFocused: true)
		)
	}
	...
	var body: some View {
			if reduceMotion {
				Text(text)
					...
					.foregroundStyle(noticeTextColor)
			} else {
				HStack(spacing: 0) {
...
private struct RightPanelView_PreviewWrapper: View {
	...
	var body: some View {
		QueuePanelContent(
			...
			resultsCountText: "\(sampleResults.count) apps processed",
		...
		.task {
			...
			while !Task.isCancelled {
				notice = .init(message: "App added to Queue", isDuplicate: false)
...
#Preview("QueuePanelContent") {
	...
	return QueuePanelContent(
		...
		resultsCountText: "1 app processed",
		queueIsEmpty: false,
...
struct InspectorSearchQueuePanelView: View {
	...
	init(
		tab: Binding<QueuePanelContent<AnyView, AnyView>.Tab>,
		notice: Binding<QueuePanelContent<AnyView, AnyView>.Notice?>,
		queueItems: Binding<[CaskApplication]>,
		resultsItems: Binding<[CaskApplication]>,
		panelMinHeight: CGFloat,
		onPrimaryAction: @escaping () -> Void,
		onSecondaryAction: @escaping () -> Void,
		onQueueItemsRemoved: (([CaskApplication]) -> Void)? = nil
	) {
		self._tab = tab
		self._notice = notice
		self._queueItems = queueItems
		self._resultsItems = resultsItems
		self.panelMinHeight = panelMinHeight
		self.onPrimaryAction = onPrimaryAction
		self.onSecondaryAction = onSecondaryAction
		self.onQueueItemsRemoved = onQueueItemsRemoved
	}
	...
	var body: some View {
		QueuePanelContent(
			...
			resultsCountText: "\(resultsItems.count) apps processed",
			queueIsEmpty: queueItems.isEmpty,
			resultsIsEmpty: resultsItems.isEmpty,
			onQueueAction: {
				let removed = queueItems
				withAnimation(.easeInOut(duration: 0.2)) {
					queueItems.removeAll()
					viewUpdateID = UUID()
				}
				onQueueItemsRemoved?(removed)
			},
			onResultsAction: {
				withAnimation(.easeInOut(duration: 0.2)) {
			...
			AnyView(
				...
				.padding(.horizontal, 10)
				.id("search-results-list-\(resultsItems.count)")
			)
		}
	...
	var body: some View {
		QueuePanelContent(
				...
				LazyVStack(spacing: 8) {
					ForEach(queueItems) { item in
						AppDetailListItem(
							...
							label: "New Version",
							onRemove: {
								let removedItems = queueItems.filter { $0.id == item.id }
								withAnimation(.easeInOut(duration: 0.2)) {
									queueItems.removeAll { $0.id == item.id }
									selectedAppKeys.remove(item.id)
									viewUpdateID = UUID()
								}
								onQueueItemsRemoved(removedItems)
							}
			...
			AnyView(
				...
				.padding(.horizontal, 10)
				.id("updates-results-list-\(resultsItems.count)")
			)
		}
		.frame(minHeight: panelMinHeight, maxHeight: .infinity, alignment: .top)
		.frame(maxWidth: .infinity, alignment: .trailing)
	...
	var body: some View {
		QueuePanelContent(
			...
			AnyView(
				...
				.padding(.horizontal, 10)
				.id("import-results-list-\(resultsItems.count)")
			)
		}
		.frame(minHeight: panelMinHeight, maxHeight: .infinity, alignment: .top)
		.frame(maxWidth: .infinity, alignment: .trailing)
