...
typealias ExpandableJuiceMenuButton_PrebigSurFallback = ExpandableJuiceMenuButtonFallback
typealias ExpandableJuiceMenuButton_AvailabilityAdapter =
	ExpandableJuiceMenuButtonAvailabilityAdapter
...
struct ExpandableJuiceMenuButton: View {
...
struct ExpandableMenuGlassSplit: View {
	...
	@State private var isPrimaryVisible = false
		...
		ZStack {
			GlassEffectContainer(spacing: glassSpacing) {
				VStack(spacing: buttonSpacing) {
					...
					if isExpanded && showsSecondaryAction {
						Button(action: handleSecondaryTap) {
							Text(secondaryTitle)
								.font(.system(size: 12, weight: .regular))
						}
					...
					ZStack(alignment: .trailing) {
						...
						if queueCount > 0 {
							Text("\(queueCount)")
								...
								.scaleEffect(
									(isEnabled && isToggleHovered) ? 1.04 : 1.0
								)
								.animation(
	...
	private func handlePrimaryTap() {
		guard isEnabled else { return }
		Self.logger.debug("Primary queue action tapped")
		onPrimary()
	...
	private func handleSecondaryTap() {
		guard isEnabled else { return }
		Self.logger.debug("Secondary queue action tapped")
		onSecondary()
		DispatchQueue.main.async {
			collapseExpanded()
		}
	...
	private func expandActions() {
		...
		expandTask = Task { @MainActor in
			...
			// Fixes the grey capsule showing before animation is completed
...
struct ExpandableJuiceMenuButtonFallback: View {
	let primaryTitle: String
	let secondaryTitle: String
	let showsSecondaryAction: Bool
	let isEnabled: Bool
...
struct ExpandableJuiceMenuButtonAvailabilityAdapter: View {
	let primaryTitle: String
	let secondaryTitle: String
	let showsSecondaryAction: Bool
	let isEnabled: Bool
	...
	var body: some View {
		if #available(macOS 26.0, *) {
			...
			ExpandableJuiceMenuButtonFallback(
				primaryTitle: primaryTitle,
				secondaryTitle: secondaryTitle,
				showsSecondaryAction: showsSecondaryAction,
				isEnabled: isEnabled,
