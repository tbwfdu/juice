...
@MainActor
@ViewBuilder
fileprivate func detailCardBackground(
...
private struct CardActionsMenu: View {
	...
	private var actionButtonShadowColor: Color {
		(colorScheme == .dark ? Color.black : Color.black).opacity(
					...
					HStack(spacing: 5) {
						if isExpanded {
							if let onDetails {
								...
								.juiceHelp(HelpText.Actions.viewDetails)
								.shadow(
							...
							if isAllExpanded, let onAddToQueue {
								...
								.controlSize(.mini)
								.buttonBorderShape(.circle)
								.juiceHelp(HelpText.Actions.addToQueue)
								.shadow(
									color: actionButtonShadowColor,
									radius: 2.2,
									x: 0,
							...
							.controlSize(.mini)
							.buttonBorderShape(.circle)
							.juiceHelp(HelpText.Actions.moreActions)
							.shadow(
								color: actionButtonShadowColor,
								radius: 2.2,
								x: 0,
...
struct AppDetailCard: View {
		...
		return VStack(alignment: .leading, spacing: 4) {
			HStack(alignment: .top, spacing: 8) {
				...
				VStack(alignment: .leading, spacing: 2) {
					Text(item.applicationName)
						...
						.juiceFullValueHelp(fullValue: item.applicationName)
					...
					Text(item.rootLocationGroupName ?? "")
						...
						.juiceFullValueHelp(
							fullValue: item.rootLocationGroupName ?? ""
						)
					HStack(spacing: 4) {
			...
			ScrollView(.horizontal, showsIndicators: false) {
				LazyHStack(spacing: 6) {
					if item.updatedApplication?.hasRecipe == true
						|| ((item.updatedApplication?.matchingRecipeId ?? "").isEmpty == false)
					{
						Pill("Recipe", style: .juiceGradient, help: HelpText.Pills.recipe)
					}
					if item.wasMatched ?? false {
						if item.hasLaterVersionInConsole ?? false {
							Pill(
								"Later Version Added",
								color: .gray,
								help: HelpText.Pills.laterVersionAdded
							)
						} else if item.hasUpdate ?? false {
							Pill("Has Update", color: .orange, help: HelpText.Pills.hasUpdate)
						} else {
							Pill("Up To Date", color: .green, help: HelpText.Pills.upToDate)
						...
						if item.wasMatched == false {
							Pill("No Matches", color: .gray, help: HelpText.Pills.noMatches)
						}
					}
					if item.status != "Active" {
						Pill("Inactive", color: .gray, help: HelpText.Pills.inactive)
					}
					...
					if assignedCount == 0 {
						Pill(
							...
							color: .blue,
							help: HelpText.Pills.assigned
						)
					}
					let smartGroupCount = item.smartGroups?.count ?? 0
					if smartGroupCount > 0 {
						Pill(
							"Smart Groups: \(smartGroupCount)",
							color: .blue,
							help: HelpText.Pills.smartGroups
						)
					}
				}
				.frame(maxWidth: .infinity, alignment: .leading)
	...
	var body: some View {
		...
		return
			...
			.padding(.vertical, 14)
				.background {
					detailCardBackground(shape: shape, colorScheme: colorScheme)
				}
...
extension View {
	...
	@ViewBuilder
	fileprivate func ifCompositingGroupAvailable26() -> some View {
		#if os(macOS)
			if #available(macOS 26.0, *) {
		...
		return VStack(alignment: .leading, spacing: 4) {
			HStack(alignment: .top, spacing: 8) {
				...
				VStack(alignment: .leading, spacing: 2) {
					Text(item.displayTitle)
						...
						.clipped()
						.font(.system(.callout, weight: .semibold))
						.juiceFullValueHelp(fullValue: item.displayTitle)
					Text(item.displaySubtitle)
						...
						.font(.system(.footnote, weight: .regular))
						.foregroundStyle(.primary)
						.juiceFullValueHelp(fullValue: item.displaySubtitle)
			...
			ScrollView(.horizontal, showsIndicators: false) {
				LazyHStack(spacing: 6) {
					...
					if item.hasMetadata {
						Pill("Metadata", color: .green, help: HelpText.Pills.metadata)
					}
					if item.macApplication != nil {
						Pill("Catalog", color: .blue, help: HelpText.Pills.catalog)
					} else {
						Pill("Filesystem", color: .gray, help: HelpText.Pills.filesystem)
					}
					Pill(fileTypeLabel, color: .orange, help: HelpText.Pills.fileType)
				}
				.frame(maxWidth: .infinity, alignment: .leading)
			}
			.frame(height: 24, alignment: .topLeading)
