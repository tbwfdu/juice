...
struct UpdatesView: View {
	private static let logger = Logger(
		...
		category: "UpdatesQueue"
	)

	// MARK: - Inputs & Environment

	let model: PageViewData
	@ObservedObject var state: UpdatesViewState
	...
	private func binding<T>(
		_ keyPath: ReferenceWritableKeyPath<UpdatesViewState, T>
	) -> Binding<T> {
		Binding(
	...
	private var queuedSourceAppsByKey: [String: UemApplication] {
		get { state.queuedSourceAppsByKey }
		nonmutating set { state.queuedSourceAppsByKey = newValue }
	}
	...
	private var uemApps: [UemApplication] {
		get { state.uemApps }
		nonmutating set { state.uemApps = newValue }
	}

	private var isQueryingUem: Bool {
		get { state.isQueryingUem }
		nonmutating set { state.isQueryingUem = newValue }
	}

	private var showAllUemApps: Bool {
		get { state.showAllUemApps }
		nonmutating set { state.showAllUemApps = newValue }
	}

	private var selectedApp: UemApplication? {
		get { state.selectedApp }
		nonmutating set { state.selectedApp = newValue }
	}

	private var selectedAppKeys: Set<String> {
		get { state.selectedAppKeys }
		nonmutating set { state.selectedAppKeys = newValue }
	}
	...
	private var isClearActionExpanded: Bool {
		get { state.isClearActionExpanded }
		nonmutating set { state.isClearActionExpanded = newValue }
	}
	...
	var body: some View {
		...
		.onChange(of: confirmationVisible) { _, isVisible in
			Self.logger.debug("Updates confirmationVisible changed: \(isVisible, privacy: .public)")
		}
	}
	...
	private func queryButtonsRow() -> some View {
		HStack(spacing: 8) {
				...
				if #available(macOS 26.0, *) {
						...
						.juiceHelp(HelpText.Updates.query)
					...
					.juiceHelp(HelpText.Updates.clear)
				} else {
							Button(action: queryAllApps) {
								HStack(spacing: 5) {
									CloudQueryBadgeIcon(
	...
	private func buttonsView() -> some View {
		ActionButtonsAvailabilityAdapter(
			...
			isClearExpanded: binding(\.isClearActionExpanded),
	...
	private func queryAllApps() {
		...
		Task { @MainActor in
			isQueryingUem = true
			let result = await UEMService.instance.getAllAppsResult()
			switch result {
			case .success(let apps):
				uemApps = apps
				selectedAppKeys.removeAll()
				selectedApp = nil
				// Widget available-updates count is API-success-backed only;
				// do not persist from transient UI state.
				persistAvailableUpdatesCount(availableUpdatesCount(in: apps))
			case .failure:
				break
			}
			isQueryingUem = false
	...
	private func appsListSection(topOverlayHeight: CGFloat = 0) -> some View {
		Group {
			if isQueryingUem {
				...
				ZStack(alignment: .bottom) {
								...
								let hasUpdate: Bool = app.hasUpdate ?? false
									...
									if #available(macOS 26.0, *) {
										...
										AppDetailCard(
											...
											onAddToQueue: hasUpdate ? { addToQueue(app) } : nil
										)
									}
								#else
									AnimatedAppCard(
					...
					if !selectedAppKeys.isEmpty {
							...
							if #available(macOS 26.0, *) {
								...
								.nativeActionButtonStyle(.secondary, controlSize: .large)
								.juiceHelp(HelpText.Updates.addSelected)
								.padding(.trailing, 20)
								.padding(.bottom, 16)
				...
				.modifier(FallbackSelectionAnimationBypass(value: selectedAppKeys.isEmpty))
				.background(Color.clear)
			} else {
				Color.clear
			}
		...
		private func updatesHeaderRow(displayedApps: [UemApplication]) -> some View
	{
		HStack(alignment: .center, spacing: 8) {
			...
			.juiceHelp(HelpText.Updates.showAll)
			.labelStyle(.iconOnly)
			.toggleStyle(.switch)
			.controlSize(.regular)
			.frame(
			...
			.padding(
				...
				if #available(macOS 26.0, *) {
					...
					.juiceHelp(HelpText.Updates.selectAll)
				} else {
					Button {
						selectAllUpdatableApps(from: displayedApps)
					} label: {
	...
	private func queuePanelView(panelMinHeight: CGFloat) -> some View {
		InspectorUpdatesQueuePanelView(
			...
			selectedAppKeys: binding(\.selectedAppKeys),
			panelMinHeight: panelMinHeight,
			onPrimaryAction: {
				Self.logger.debug("Updates queue primary action invoked")
				confirmationMode = .upload
				confirmationVisible = true
			},
			onSecondaryAction: {
				Self.logger.debug("Updates queue secondary action invoked")
	...
	private var availableUpdatesCount: Int {
		availableUpdatesCount(in: uemApps)
	}

	private func availableUpdatesCount(in apps: [UemApplication]) -> Int {
		apps.reduce(0) { partialResult, app in
			partialResult + ((app.hasUpdate ?? false) ? 1 : 0)
		}
	}

	private func persistAvailableUpdatesCount(_ count: Int) {
		var state = settingsStore.load()
		guard state.availableUpdatesCount != count else { return }
		state.availableUpdatesCount = count
...
private struct FallbackSelectionAnimationBypass<Value: Equatable>: ViewModifier {
	let value: Value
	func body(content: Content) -> some View {
		if #available(macOS 26.0, *) {
