...
final class DownloadQueueViewModel: ObservableObject {
	...
	private func uploadEditedDownloads() async {
		...
		for editable in editableDownloads {
			...
			statusText = "Uploading Files"
			updateUploadStatus(prepared.macApplication ?? editable.baseDownload.macApplication, inProgress: true, status: "Preparing to upload")
			...
			updateUploadStatus(prepared.macApplication, inProgress: true, status: "Uploading metadata...")
			do {
				let uploadOK = try await DownloadUploadService.uploadSuccessfulDownload(
					...
					onStatus: { [weak self] status in
						self?.updateUploadStatus(prepared.macApplication, inProgress: true, status: status)
					}
				)
				if uploadOK {
					successCount += 1
