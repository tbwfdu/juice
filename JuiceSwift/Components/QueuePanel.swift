...
struct QueuePanel<QueueContent: View, ResultsContent: View>: View {
    enum Tab: Hashable {
        case queue
        case results
    }
	...
	private var panelGlassOpacity: CGFloat { focusObserver.isFocused ? 0.99 : 0.9 }
	private var panelGlassBaseOpacity: CGFloat { focusObserver.isFocused ? 0.62 : 0.3 }
	...
	var body: some View {
		let glassBaseOpacity = panelGlassBaseOpacity
		let glassOpacity = panelGlassOpacity
		...
		ZStack(alignment: .center) {
			VStack(alignment: .leading, spacing: 12) {
				...
				Picker("", selection: $tab) {
					...
					Text("Results").tag(Tab.results).disabled(!resultsTabEnabled)
				}
			...
			if tab == .queue, let notice = displayNotice {
				HStack(spacing: 8) {
					Image(systemName: notice.isDuplicate ? "exclamationmark.triangle.fill" : "checkmark.circle.fill")
				...
				.transaction { $0.animation = MacNotificationAnimation.animation }
	...
	private func panelContainer<Content: View>(
		...
		ElevatedPanel(
			...
			glassBaseOpacity: panelGlassBaseOpacity
		) {
			content()
		}
	}
	...
	private func tabTransition(for tab: Tab) -> AnyTransition {
		let insertion = AnyTransition.opacity
			.combined(with: .modifier(active: BlurFadeTransitionState(amount: 5), identity: BlurFadeTransitionState(amount: 0)))
...
private extension QueuePanel {
	@MainActor
	func beginNoticeDismiss() {
		withAnimation(MacNotificationAnimation.animation) {
			isDismissingNotice = true
		}
		noticeTask?.cancel()
		noticeTask = Task { @MainActor in
			try? await Task.sleep(nanoseconds: MacNotificationAnimation.exitDuration)
			displayNotice = nil
			notice = nil
			isDismissingNotice = false
		}
...
private enum MacNotificationAnimation {
	...
	static var transition: AnyTransition {
		.asymmetric(
			insertion: .modifier(
				active: NotificationTransitionState(opacity: 0, scale: 0.985, blur: 6, offsetY: -12),
				identity: NotificationTransitionState(opacity: 1, scale: 1, blur: 0, offsetY: 0)
			),
			removal: .identity
		)
	}
					...
					Text(String(character))
						...
						.animation(
							.interpolatingSpring(stiffness: 260, damping: 16)
								.delay(Double(index) * MacNotificationAnimation.entryStagger),
							value: animate
						)
				}
			}
...
private struct GlassSegmentedControl<Tag: Hashable>: View {
	...
	private let segmentPadding = EdgeInsets(top: 9, leading: 16, bottom: 9, trailing: 16)
    ...
    var body: some View {
        ...
        .background {
            ...
            if #available(macOS 26.0, iOS 26.0, *) {
				...
				.overlay {
					shape
						.fill(Color.white.opacity(backgroundGlassBaseOpacity))
		...
		.animation(.timingCurve(0.22, 0.88, 0.3, 1.0, duration: 0.32), value: selection)
    }
    ...
    private func segmentButton(for item: Item) -> some View {
        ...
        let backgroundOpacity: CGFloat = isPressed ? 0.16 : (isHovered ? 0.12 : 0)
        ...
        .background {
			if isSelected {
				segmentSelectionPill
					.matchedGeometryEffect(id: "segmentedSelection", in: selectionNamespace)
        ...
        .onLongPressGesture(minimumDuration: 0.01, maximumDistance: 12, pressing: { pressing in
...
private struct SegmentedActionButton: View {
	...
	private let padding = EdgeInsets(top: 9, leading: 16, bottom: 9, trailing: 16)
...
private struct RightPanelView_PreviewWrapper: View {
    @State private var tab: QueuePanel<AnyView, AnyView>.Tab
	@State private var notice: QueuePanel<AnyView, AnyView>.Notice?
	...
	init(
		tab: QueuePanel<AnyView, AnyView>.Tab = .queue,
		notice: QueuePanel<AnyView, AnyView>.Notice? = nil,
...
#Preview("RightPanelView Notice Animation") {
	RightPanelView_PreviewWrapper(tab: .queue, notice: nil, showsNoticeDemo: true)
}
